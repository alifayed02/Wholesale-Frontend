...
interface CloseRateGaugeProps {
  value: number; // 0-1
}

const CloseRateGauge: React.FC<CloseRateGaugeProps> = ({ value }) => {
  const percent = Math.round(value * 100);
  const data = [
    { name: 'Close Rate', value: percent, fill: 'url(#glowRed)' },
  ...
  return (
    <div className="relative w-full h-56 bg-neutral-900 rounded-3xl shadow-xl flex flex-col items-center justify-center p-6 overflow-hidden">
      <div className="text-white text-lg font-bold tracking-wide mb-2 font-sans">Close Rate</div>
